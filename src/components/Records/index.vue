...
function handleBlur(event: any) {
  const isMenuItemButton =
    event.relatedTarget &&
    event.relatedTarget.classList.contains('record-menu-item');
...
<template>
  <header class="flex justify-center mx-auto mt-8 sm:mb-10">
    <h1>{{ folderStore.activeFolderName }}</h1>
  </header>

  <div class="ml-10 mb-4 sm:mb-10">
    <RouterLink
      :to="{
        name: 'category',
        params: {
  ...
  <div class="flex justify-center mb-40 relative">
    <div class="bg-white/[.50] p-2 sm:p-4 w-full md:w-[90vw] shadow-lg">
      <component
      ...
      <div class="inline-block relative">
        <AddButton @on-click="toggleAddButton" @on-blur="handleBlur" />
        <AddRecordMenu
          v-if="showAddRecordMenu"
